...
void print_stat(const std::string &method, const int size, const double flops, const double secs, const double fnorm)
{
...
*/
__global__ void reduceAtomicShared(const ValueType* input, int N)
{
    const int id = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void reduceShared(const ValueType* input, int N)
{
    const int id = threadIdx.x + blockIdx.x * blockDim.x;
...
void test_host(int size, int nrepeat)
{
    matrix mat(size);
    fill(mat);
...
void test_kokkos(const int M, const int N, const int nrepeat)
{
  ...
  for ( int repeat = 0; repeat < nrepeat; repeat++ ) {
    ...
    Kokkos::parallel_reduce( "Frobenius norm", range_policy( 0, N ), KOKKOS_LAMBDA ( int j, double &update ) {
      double temp2 = 0;
...
//           N, M, nrepeat, Gbytes * 1000, time, Gbytes * nrepeat / time );
  ...
  void test_cuda(const int M, const int N, const int nrepeat)
{
    ...
    cudaMemcpy(d_mat, mat, sizeof(ValueType) * matrix_length, cudaMemcpyHostToDevice);
